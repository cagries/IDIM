...
class BaseSet(Dataset):
    def __init__(self, mode="train", sampler=None, train_json=None, valid_json=None, transform=None):
        self.mode = mode
        ...
        if self.mode == "train":
            ...
            self.json_path = train_json
        elif self.mode == "valid":
            print("Loading valid data ...", end=" ")
            self.json_path = valid_json
        else:
        ...
        if sampler != None and mode == "train":
            print('-'*20+' dataset'+'-'*20)
    ...
    def get_targets(self):
        return [data['category_id'] for data in self.data]

    def update_transform(self, input_size=None):
        normalize = TRANSFORMS["normalize"](input_size=input_size)
        transform_list = [transforms.ToPILImage()]
        transform_ops = (
            self.transform
        )
        for tran in transform_ops:
            transform_list.append(TRANSFORMS[tran](input_size=input_size))
...
class iNaturalist(BaseSet):
    def __init__(self, mode='train', sampler=None, train_json=None, valid_json=None, transform=None):
        super(iNaturalist, self).__init__(mode, sampler, train_json, valid_json, transform)
        random.seed(0)
        self.class_dict = self._get_class_dict()
        self.sampler = sampler
    def __getitem__(self, index):
        if self.sampler != None and self.mode == 'train':
            assert self.sampler in ["balance", 'square', 'progressive']
            if  self.sampler == "balance":
                sample_class = random.randint(0, self.num_classes - 1)
            elif self.sampler == "square":
    ...
    def get_cls_num_list(self):
        ...
        num_classes = [0 for i in range(1000)]
        for t in targets:
            num_classes[t]+=1
        return num_classes
