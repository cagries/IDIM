...
parser.add_argument('--margin_type', type=str, choices=['learned', 'cardinality', 'id'], default="learned")
parser.add_argument('--balanced_clf_nepochs', default=20, type=int)
parser.add_argument('--id_values', nargs="*", default=None, type=float) 

best_acc1 = 0


def freeze_layers(model, fe_bool=True, cls_bool=True, gpu=None):
    print("** Freeze layers with fe_bool: {}, cls_bool: {}".format(fe_bool, cls_bool)) 
    if fe_bool:
        model.train()
    else:
        model.eval()
    if cls_bool:
        if gpu is not None:
            ...
            model.module.linear.train()
    else:
        if gpu is not None:
            model.linear.eval()
        else:
            model.module.linear.eval()
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.epochs == args.balanced_clf_nepochs:
        print("Create Optimizer")
        cls_optim = torch.optim.SGD(feat_params + cls_params, args.cls_lr,
                                    momentum=args.momentum,
                                    weight_decay=args.weight_decay)
        if args.dataset == "imagenet":
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(cls_optim, args.epochs, eta_min=0, verbose=True)
    else:
        print("Create Optimizer")
    ...
    if args.resume or args.evaluation:
        ...
        if os.path.isfile(filename):
            ...
            if args.dataset == "imagenet" and (args.balanced_clf_nepochs == args.epochs):
                cls_optim.load_state_dict(checkpoint['optimizer'])
    ...
    # create losses
    def create_loss_list(args_str, args, num_classes): 
        loss_ls = []
        ...
        for loss_str in loss_str_ls:
            ...
            if loss_str == "ce":
                ...
                if args.margin_type == "cardinality":
                    card_epsilons = torch.FloatTensor([args.eps/math.sqrt(card) for card in cls_num_list]) 
                    card_epsilons = card_epsilons / torch.sum(card_epsilons) * num_classes
                    loss_ls += [DROLoss(temperature=args.temperature,
                                    base_temperature=args.temperature,
                                    class_weights=c_weights,
                                    epsilons=card_epsilons)]
                elif args.margin_type == "id":
                    id_epsilons = torch.FloatTensor(args.id_values)
                    loss_ls += [DROLoss(temperature=args.temperature,
                                    base_temperature=args.temperature,
                                    class_weights=c_weights,
                                    epsilons=id_epsilons)]
                else: 
                    loss_ls += [DROLoss(temperature=args.temperature,
                                    base_temperature=args.temperature,
                                    class_weights=c_weights,
    ...
    feat_losses = create_loss_list(args.feat_loss,args,num_classes)
    cls_losses = create_loss_list(args.cls_loss,args,num_classes)
    ...
    train_features, train_targets = None, None
    print('Training will start, took {:.2f}s'.format(time.time() - script_time))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch < args.epochs - args.balanced_clf_nepochs:
            ...
            freeze_layers(model, fe_bool=True, cls_bool=False, gpu=args.gpu)
            train(feat_train_loader, model, feat_losses, epoch, feat_optim, args, train_features, train_targets)

        else:
            if epoch == args.epochs - args.balanced_clf_nepochs and epoch:
            ...
            if args.balanced_clf_nepochs == args.epochs:
                freeze_layers(model, fe_bool=True, cls_bool=True, gpu=args.gpu)
            else:
                freeze_layers(model, fe_bool=False, cls_bool=True, gpu=args.gpu)
            train(feat_train_loader, model, cls_losses, epoch, cls_optim, args)
            if args.balanced_clf_nepochs == args.epochs:
                if args.dataset != "imagenet":
                    adjust_learning_rate(cls_optim, epoch, args)
                else:
                    scheduler.step()
        print("=============== Extract Train Centroids ===============")
        ...
        print('Epoch {:d}, took {:.2f}s'.format(epoch, time.time() - training_start_time))
