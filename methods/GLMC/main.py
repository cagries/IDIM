...
def main_worker(gpu, args):
    ...
    #weighted_loader
    if args.id_sampling_weights_path:
        cls_weight = np.load(args.id_sampling_weights_path) ** args.resample_weighting
        print('Using ID sampling weights from file:', args.id_sampling_weights_path)
    ...
    trainer = Trainer(args, model=model,train_loader=train_loader, val_loader=val_loader,weighted_train_loader=weighted_train_loader, per_class_num=train_cls_num_list,log=logging, id_weights_path=args.id_weights_path)
    trainer.train()
    end_time = time.time()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--id_weights_path', type=str) # 'output/')
    parser.add_argument('--id_sampling_weights_path', type=str) # 'output/')
    main()
