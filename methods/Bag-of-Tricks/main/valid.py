...
def valid_model(dataLoader, model, cfg, device, num_classes):
    ...
    if 'CIFAR' in cfg.DATASET.DATASET:
        # Calculate F-C-R for CIFAR-like datasets
        matrix = fusion_matrix.matrix
        ...
        for i in range(matrix.shape[0]):
            ...
            accs.append(acc)
        accs = np.array(accs)
        if cfg.DATASET.DATASET == 'IMBALANCECIFAR10':
            # 3-4-3 split
            freq = accs[:3]
            common = accs[3:3 + 4]
            rare = accs[3 + 4:]
            print(f'F & C & R | Average: {freq.mean():.3} & {common.mean():.3} & {rare.mean():.3} | {accs.mean():.4}')
        elif cfg.DATASET.DATASET == 'IMBALANCECIFAR100':
            freq = accs[:33]
            common = accs[33:33 + 34]
            rare = accs[33 + 34:]
            print(f'F & C & R | Average: {freq.mean():.3} & {common.mean():.3} & {rare.mean():.3} | {accs.mean():.4}')
...
if __name__ == "__main__":
    ...
    model_file = cfg.OUTPUT_DIR + '/' + cfg.NAME + '/models/' + cfg.TEST.MODEL_FILE
