...
if __name__ == "__main__":
    ...
    if local_rank == 0:
        ...
        if not os.path.exists(model_dir):
            ...
            logger.info(
                "This directory has already existed, former directory will be overwritten."
            )
    ...
    if cfg.TRAIN.DISTRIBUTED:
        ...
        if cfg.DATASET.DATASET != "CIFAR":
            testLoader = DataLoader(
                test_set, batch_size=cfg.TEST.BATCH_SIZE, shuffle=False,
                num_workers = cfg.TEST.NUM_WORKERS, pin_memory=False,
    ...
    for epoch in range(start_epoch, epoch_number + 1):
        ...
        if cfg.VALID_STEP != -1 and epoch % cfg.VALID_STEP == 0:
            ...
            if valid_acc > best_result and local_rank == 0:
                ...
                if cfg.DATASET.DATASET != 'CIFAR':
                    # Also keep track of test set performance for best model
                    test_acc, test_loss = valid_model(
                        testLoader,
                        epoch,
                        model,
                        cfg,
                    ...
                    loss_dict["test_loss"], acc_dict["test_acc"] = test_loss, test_acc

            if rank == 0:
                logger.info(
                    "--------------Best_Epoch:{:>3d}    Best_Valid_Acc:{:>5.2f}%--------------".format(
                        best_epoch, best_result * 100
                    )
                )
                if cfg.DATASET.DATASET != 'CIFAR':
                    logger.info(
                        "--------------Best_Epoch:{:>3d}    Best_Test_Acc:{:>5.2f}%--------------".format(
                            best_epoch, test_acc * 100
                        )
                    )
    ...
    if rank == 0:
        logger.info(
            "-------------------Train Finished :{} (Seed: {}) -------------------".format(cfg.NAME, args.seed)
        )
