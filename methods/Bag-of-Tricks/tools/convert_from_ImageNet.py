...
def convert(input_path, image_root):
    ...
    valid = open(os.path.join(input_path, 'ImageNet_LT_val.txt')).readlines()
    test = open(os.path.join(input_path, 'ImageNet_LT_test.txt')).readlines()
    train_annos = []
    valid_annos = []
    test_annos = []
    ...
    for info in tqdm(train):
        ...
        idx += 1
    print("Converting file {} ...".format(os.path.join(input_path, 'ImageNet_LT_val.txt')))
    idx = 0
    ...
    idx = 0
    for info in tqdm(test):
        image, category_id = info.strip().split(' ')
        test_annos.append({"image_id": idx,
                            "category_id": int(category_id),
                            "fpath": os.path.join(image_root, image)
                            })
        idx += 1
    num_classes = 1000
    return {"annotations": train_annos, "num_classes": num_classes}, {'annotations': valid_annos, "num_classes": num_classes}, {"annotations": test_annos, "num_classes": num_classes}

if __name__ == "__main__":
    args = parse_args()
    train_annos, valid_annos, test_annos = convert(args.input_path, args.image_path)
    print("Converted, Saving converted file to {}".format(args.output_path))
    ...
    with open(os.path.join(args.output_path, 'ImageNet_LT_test.json'), "w") as f:
