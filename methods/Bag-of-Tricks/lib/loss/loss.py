...
class CrossEntropy(nn.Module):
    def __init__(self, para_dict=None):
        ...
        if self.para_dict['cfg'].LOSS.CrossEntropy.WEIGHTS:
            print('Using weights:', self.para_dict['cfg'].LOSS.CrossEntropy.WEIGHTS)
            self.weight_list = torch.FloatTensor(self.para_dict['cfg'].LOSS.CrossEntropy.WEIGHTS).to(self.device)
        else:
    ...
    def forward(self, inputs, targets, **kwargs):
        ...
        """
        if self.weight_list != None:
            weight = (self.weight_list[targets]).to(targets.device)
            loss = F.cross_entropy(inputs, targets, reduction='none')
            
            # Instead of mean, we will normalize the loss by avg_factor to keep loss magnitude similar
            avg_factor = weight.sum()

            # For stability, we limit the average factor to a small number
            if avg_factor < EPSILON:
                avg_factor = EPSILON

            # Multiply each loss with the corresponding id-based weight and normalize by avg_factor
...
class FocalLoss(CrossEntropy):
    ...
    def forward(self, inputs, targets, **kwargs):
        ...
        # Computing multi-class focal loss
        log_p = torch.nn.functional.log_softmax(inputs, dim=1)
        ce = self.nll(log_p, targets)   # this is -log(pt)
        all_rows = torch.arange(inputs.shape[0])
        log_pt = log_p[all_rows, targets] # this is pt
        pt = log_pt.exp()
        focal_weights = torch.pow(1 - pt, self.gamma) # and this is (1-pt)^gamma
        loss = ce * focal_weights * self.weight_list.view(-1, 1)
        loss = loss.mean()
...
class ClassBalanceFocal(CrossEntropy):
    ...
    def forward(self, inputs, targets, **kwargs):
        ...
        # print("CB weights:", weight)
        label = get_one_hot(targets, self.num_classes)
        # p = self.sigmoid(inputs)
        # class_prediction = (F.softmax(inputs, dim=1)).max(1)[1]
        # Record classwise accuracy
        # classwise_accuracy = [0 for i in range(self.num_classes)]
        # classwise_count = [0 for i in range(self.num_classes)]
        # for idx in range(targets.shape[0]):
        #     if (targets[idx].item()==class_prediction[idx].item()):
        #         classwise_accuracy[targets[idx].item()] += 1
        #     classwise_count[targets[idx].item()] += 1
        # classwise_accuracy = [round(x/y, 4) if y!=0 else 0 for x, y in zip(classwise_accuracy, classwise_count)]
        ...
        pt = log_pt.exp()
        focal_weights = torch.pow(1 - pt, self.gamma) # and this is (1-pt)^gamma
        loss = (ce * focal_weights * weight).mean()
...
class IdFocal(CrossEntropy):
    r"""
    Reference:
    Li et al., Focal Loss for Dense Object Detection. ICCV 2017.

        Equation: Loss(x, class) = u_c * FocalLoss(x, c)

    Class-based ID considers the ID of the model for each class, \
    rather than nominal number of images provided by original datasets.

    Args:
        id_weights: ID weights for each class with shape (num_classes)
        gamma(float, double) : gamma > 0; reduces the relative loss for well-classiﬁed examples (p > .5),
                               putting more focus on hard, misclassiﬁed examples
    """
    def __init__(self, para_dict=None):
        super(IdFocal, self).__init__(para_dict)
        self.id_weights = np.array(self.para_dict['cfg'].LOSS.IdFocal.id_WEIGHTS)
        self.id_weights = torch.FloatTensor(self.id_weights)
        self.gamma = self.para_dict['cfg'].LOSS.IdFocal.GAMMA
        # self.sigmoid = nn.Sigmoid()
        self.nll = nn.NLLLoss(reduction='none')
    ...
    def update(self, epoch):
        ...
        """
        if not self.drw:
            self.weight_list = self.id_weights
        else:
            start = (epoch-1) // self.drw_start_epoch
            if start:
                self.weight_list = self.id_weights
            else:
                self.weight_list = torch.ones(self.id_weights.shape).to(self.device)
        if DEBUG:
            print('*'*100)
            print(self.weight_list)
            print(self.drw)
...
class CBIdFocal(CrossEntropy):
    ...
    Class-based ID considers the amount of ID of the model for each class, \
    rather than nominal number of images provided by original datasets.
    ...
    """
    def __init__(self, para_dict=None):
        super(CBIdFocal, self).__init__(para_dict)
        self.id_weights = np.array(self.para_dict['cfg'].LOSS.CBIdFocal.id_WEIGHTS)
        self.beta = self.para_dict['cfg'].LOSS.ClassBalanceFocal.BETA
        self.class_balanced_weight = np.array([(1-self.beta)/(1- self.beta ** N) for N in self.num_class_list])
        self.class_balanced_weight = torch.FloatTensor(self.class_balanced_weight / np.sum(self.class_balanced_weight) * self.num_classes).to(self.device)

        self.id_weights = torch.FloatTensor(self.id_weights / np.sum(self.id_weights)  * self.num_classes).to(self.device)

        self.f = self.para_dict['cfg'].LOSS.CBIdFocal.FACTOR

        self.id_weights = self.f*self.id_weights+(1-self.f)*self.class_balanced_weight
        self.gamma = self.para_dict['cfg'].LOSS.CBIdFocal.GAMMA
        # self.sigmoid = nn.Sigmoid()
        self.nll = nn.NLLLoss(reduction='none')
...
class CBID(CrossEntropy):
    r"""
    Class-balanced cross-entropy, together with ID weights.

    Args:
        id_weights: ID weights for each class with shape (num_classes)
    """
    def __init__(self, para_dict=None):
        super(CBID, self).__init__(para_dict)
        self.id_weight = torch.FloatTensor(np.array(self.para_dict['cfg'].LOSS.CBID.WEIGHTS)).to(self.device)
        self.beta = self.para_dict['cfg'].LOSS.CBID.BETA
        self.class_balanced_weight = np.array([(1-self.beta)/(1- self.beta ** N) for N in self.num_class_list])
        self.class_balanced_weight = torch.FloatTensor(self.class_balanced_weight / np.sum(self.class_balanced_weight) * self.num_classes).to(self.device)

        self.f = self.para_dict['cfg'].LOSS.CBID.FACTOR

        self.weights = self.f * self.id_weight + (1-self.f) * self.class_balanced_weight
    ...
    def update(self, epoch):
        ...
        """
        if not self.drw:
            self.weight_list = self.weights
        else:
            start = (epoch-1) // self.drw_start_epoch
            if start:
                self.weight_list = self.weights
            else:
                self.weight_list = torch.ones(self.weights.shape).to(self.device)
        if DEBUG:
            print('*'*100)
            print(self.weight_list)
            print(self.drw)
