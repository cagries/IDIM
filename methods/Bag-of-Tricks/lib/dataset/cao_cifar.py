...
class IMBALANCECIFAR10(torchvision.datasets.CIFAR10):
    ...
    def __init__(self, mode, cfg, root='~/dataset/cifar', imb_type='exp',
        ...
        if self.cfg.TRAIN.SAMPLER.TYPE == "weighted sampler" and self.train:
            ...
            self.class_dict = self._get_class_dict()
            if self.cfg.TRAIN.SAMPLER.WEIGHTED_SAMPLER.TYPE == "id-based":
                self.id_p = np.array(self.cfg.TRAIN.SAMPLER.WEIGHTED_SAMPLER.IdBased.PROBS)
                self.id_p = self.id_p / np.sum(self.id_p)
    ...
    def update(self, epoch):
        ...
        if self.cfg.TRAIN.SAMPLER.WEIGHTED_SAMPLER.TYPE == "id-based" and \
                self.cfg.TRAIN.SAMPLER.WEIGHTED_SAMPLER.IdBased.PROGRESSIVE:
            self.progress_p = (
                    epoch / self.cfg.TRAIN.MAX_EPOCH * self.id_p \
                    + (1 - epoch / self.cfg.TRAIN.MAX_EPOCH) * self.instance_p
            )
            # print('self.progress_p', self.progress_p)
    ...
    def __getitem__(self, index):
        ...
        if self.cfg.TRAIN.SAMPLER.TYPE == "weighted sampler" and self.train \
            ...
            elif self.cfg.TRAIN.SAMPLER.WEIGHTED_SAMPLER.TYPE == "id-based":
                if self.cfg.TRAIN.SAMPLER.WEIGHTED_SAMPLER.IdBased.PROGRESSIVE:
                    sample_class = np.random.choice(
                        np.arange(self.cls_num), p=self.progress_p)
                else:
                    probs = self.cfg.TRAIN.SAMPLER.WEIGHTED_SAMPLER.IdBased.PROBS
                    sample_class = np.random.choice(
                        np.arange(self.cls_num), p=[p_ / sum(probs) for p_ in probs]
                    )
            else:  # progressive
