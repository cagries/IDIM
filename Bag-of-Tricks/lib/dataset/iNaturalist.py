...
class iNaturalist(BaseSet):
    ...
    def __getitem__(self, index):
        if self.cfg.TRAIN.SAMPLER.TYPE == "weighted sampler" \
            ...
            assert self.cfg.TRAIN.SAMPLER.WEIGHTED_SAMPLER.TYPE in ["balance", 'square', 'progressive', 'id-based']
            if  self.cfg.TRAIN.SAMPLER.WEIGHTED_SAMPLER.TYPE == "balance":
                ...
                if self.cfg.TRAIN.SAMPLER.WEIGHTED_SAMPLER.IdBased.PROGRESSIVE:
                    ...
                    sample_class = np.random.choice(np.arange(self.num_classes), p=[p_/sum(probs) for p_ in probs])
            else: # progressive sampling
                sample_class = np.random.choice(np.arange(self.num_classes), p=self.progress_p)
