...
from backbone import (res32_cifar, res50, res10, res152, res152_no_caffe, res152_no_pooling)
    ...
    def get_CAM_with_groundtruth(self, image_idxs, dataset, label_list, size):
        ...
        for i in range(len(image_idxs)):
            ...
            with torch.no_grad():
                ...
                # print('Image dims before forward', img.shape)
                feature_conv = self.forward(img.to('cuda'), feature_maps_flag=True).detach().cpu().numpy()
            # Print the shape
            # print('Backbone output:', feature_conv.shape)
