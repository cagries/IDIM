...
def res152_no_caffe( cfg, pretrain=True, pretrained_model="", last_layer_stride=2):
    rn = tm.resnet152(weights='IMAGENET1K_V2')  # Use the best possible weights
    # Omit FC:
    rn.fc = nn.Identity()
    #rn = nn.Sequential(*(list(rn.children())[:-1]))  # alternative for omitting fc, untested
    return rn

from types import MethodType

def res152_no_pooling( cfg, pretrain=True, pretrained_model="", last_layer_stride=2):
    rn = tm.resnet152(weights='IMAGENET1K_V2')  # Use the best possible weights
    # ignoring the final layers here, to get CAM features
    def _forward_impl_no_pooling(self, x):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        ...
        # x = self.fc(x)
        return x
    # rn.avgpool = nn.Identity()
    # rn.fc = nn.Identity()
    rn._forward_impl = MethodType(_forward_impl_no_pooling, rn)
    return rn
