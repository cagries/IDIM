...
flags.DEFINE_float('lr', 0.1, 'learning rate parameter for logit adjustment.')
...
def main(_):
  ...
  best_posthoc_acc = 0.0

  # Model to be trained.
  model = models.cifar_resnet32(dataset.num_classes)

  # Read the ID probabilities to use for logit adjustment.
  id_weights = 'fish-cifar10-lt.txt'
  if 'cifar100' in FLAGS.dataset:
    id_weights = 'fish-cifar100-lt.txt'

  base_probs_path = os.path.join(FLAGS.data_home, id_weights)

  print('Using base probs from', base_probs_path)
  ...
  for epoch in tqdm(range(dataset.num_epochs)):
    ...
    if posthoc_adjusting:
      ...
      print(f'Logit-adjusted test accuracy (epoch {epoch}): {test_adj_acc:.4f}')
      if best_posthoc_acc < test_adj_acc:
          best_posthoc_acc = test_adj_acc
          print(f'New high score: {best_posthoc_acc:.4f}')
  ...
  print('Best score', best_posthoc_acc)
