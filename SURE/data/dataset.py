...
def TrainDataLoader(img_dir, transform_train, batch_size, is_train=True, dataset_type='normal', imb_factor=None, id_sampling_weights_path=None, id_sampling_scale=1.0):
    train_set = CustomImageFolder(img_dir, transform_train, is_train=is_train, dataset_type=dataset_type, imb_factor=imb_factor)
    if id_sampling_weights_path is not None:
        print('Using ID sampling weights from file:', id_sampling_weights_path)
        print('Using ID sampling scaling:', id_sampling_scale)
        id_scaling = id_sampling_scale
        cls_weight = np.load(id_sampling_weights_path) ** id_scaling
        print('ID sampling weights:', cls_weight)
        samples_weight = np.array([cls_weight[t] for t in train_set.targets])
        samples_weight = torch.from_numpy(samples_weight)
        samples_weight = samples_weight.double()
        weighted_sampler = WeightedRandomSampler(samples_weight, len(samples_weight),replacement=True)
        weighted_train_loader = DataLoader(train_set, batch_size=batch_size, num_workers=4, persistent_workers=True,pin_memory=True,sampler=weighted_sampler)
...
def get_loader(dataset, train_dir, val_dir, test_dir, batch_size, imb_factor, model_name, id_sampling_weights_path=None, id_sampling_scale=1.0):
    ...
    train_loader = TrainDataLoader(train_dir, transform_train, batch_size, is_train=True, dataset_type=dataset, imb_factor=imb_factor, id_sampling_weights_path=id_sampling_weights_path, id_sampling_scale=id_sampling_scale)
